...
import com.sc.mf919pro.kotlin.activity.GiftBoxResultDialogFragment
import com.sc.mf919pro.kotlin.activity.LuckyDrawDialogFragment
...
class FragmentReceipt : BaseFragment() {
    ...
    override fun onDestroyView() {
        ...
        if (this::helperLog.isInitialized && this::helperLogClassName.isInitialized) {
            helperLog.appendLine(helperLogClassName, "FragmentReceipt OnDestroyView :: screen ended")
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        requireActivity().onBackPressedDispatcher.addCallback(viewLifecycleOwner,
            object : OnBackPressedCallback(true) {
                override fun handleOnBackPressed() { listener?.fragmentReceiptBackAction() }
            }
        )
        ...
        if(DbModelTerminalConfig.getBooleanValue(dbModelTerminalConfig, "UNATTENDED_MODE")) {
            ...
            buttonGroupRight.setDebouncedOnClickListener {
                ...
                printReceipt(null, details, "CUSTOMER", bundleValue)
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            if (transData.salesType == 0 || DbModelTerminalConfig.getBooleanValue(dbModelTerminalConfig, "ReceiptPrint")) {
                ...
                printReceipt(null, details, "MERCHANT", bundleValue)
            }
    ...
    //TODO Demo POC only
    private fun showLuckyDraw() {
        val prizeList = listOf(
            LuckyDrawItem(1, "50% On Next Visit"),
            LuckyDrawItem(2, "50% On Next Visit"),
            LuckyDrawItem(3, "50% On Next Visit"),
            LuckyDrawItem(4, "50% On Next Visit")
        )

        val dialog = LuckyDrawDialogFragment.newInstance(prizeList)
        dialog.listener = object : LuckyDrawDialogFragment.Listener {
            override fun onPrizeSelected(prize: String) {
                //startTimer(0)
                GiftBoxResultDialogFragment
                    .newInstance(prize)
                    .show(parentFragmentManager, "GiftBoxResultDialogFragment")
            }
        }

        dialog.show(parentFragmentManager, "LuckyDrawDialogFragment")
    }
