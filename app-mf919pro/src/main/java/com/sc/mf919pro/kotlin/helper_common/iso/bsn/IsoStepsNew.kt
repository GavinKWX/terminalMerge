package com.sc.mf919pro.kotlin.helper_common.iso.bsn
...
import com.sc.mf919pro.java.activity.IsoUtil
...
import com.sc.mf919pro.kotlin.helper_common.iso.IsoInfoModel
...
object IsoStepsBSNNew {
    ...
    fun transformBatchData(context: Context){
        ...
        //TransData.removeTlvFromTransDb("BF38") //BSN VOID CANNOT remove
    ...
    fun formIsoMessage(context: Context, helperLog: HelperLog, isoInfoModel: IsoInfoModel, destByteOffset: Int, isoByte: ByteArray, byteArrayLen: IntArray) {
        ...
        if(TransData.eppTenureCode.isNotEmpty()) {
            ...
            TransData.removeTlvFromTransDb("DF60")
            TransData.addHexStrIntoTransDB("DF60", HexUtil.str2HexStr(TransData.eppTenureCode))
        }

        if (TransData.txnTypeLabel == "Void Instalment"){
            val productModel = ProductListRepo.getSingle(context, listOf("AcqMid", "AcqTid"), arrayOf(TransData.mid, TransData.tid))
            if (productModel != null) {
                TransData.ksn = productModel.Ksn
                TransData.pinKsn = productModel.PinKsn
                TransData.product = productModel.Product
                helperLog.appendLine(logClassName, "Changed Product :: ${TransData.product}")
                helperLog.appendLine(logClassName, "Changed EPP Ksn :: ${TransData.ksn}")
            }
        }
        ...
        if (TransData.onlinePinInput || TransData.offlinePinInput) {
            var newPinKsn = ""
            try {
                newPinKsn = Dukpt.getNewKsn(TransData.pinKsn)
                ProductListRepo.updateData(context, hashMapOf("PinKsn" to newPinKsn), hashMapOf("Product" to TransData.product, "AcqMid" to TransData.mid, "AcqTid" to TransData.tid))
                newPinKsn = newPinKsn.substring(4)
            }catch (ex: Exception) {
                ex.printStackTrace()
            }
            helperLog.appendLine(logClassName, "Old Pin Ksn :: ${TransData.pinKsn}")
            helperLog.appendLine(logClassName, "New Pin Ksn :: $newPinKsn")
            TransData.addHexStrIntoTransDB("DF53", newPinKsn)
        }

        helperLog.appendLine(logClassName, "Old Ksn :: ${TransData.ksn}")
        var newKsn = ""
        try{
            newKsn = Dukpt.getNewKsn(TransData.ksn)
            ProductListRepo.updateData(context, hashMapOf("Ksn" to newKsn), hashMapOf("Product" to TransData.product, "AcqMid" to TransData.mid, "AcqTid" to TransData.tid))
        }catch (e: Exception){
            e.printStackTrace()
        }
        ...
        if(encDesField[0] != 0){
                ...
                if(iThisLen > 0){
                    ...
                    when(iThisField){
                        35 -> {
                            ...
                            println("tt :: $tt")*/
                            strTrack2 = Utils.paddingWith(iThisField.toString(), "0", 3, false) + "0" + Utils.paddingWith(strTrack2.length.toString(), "0", 3, false) + strTrack2
                        ...
                        2, 14, 45, 48, 52, 55, 58, 60, 63 -> {
                           /*
                            ...
                            val newSenString = Utils.paddingWith(iThisField.toString(), "0", 3, false) + de57EncMethod + Utils.paddingWith(tempSenString.length.toString(), "0", 3, false) + tempSenString
                           ...
                           * */
                            ...
                            if (iThisField == 2 || iThisField == 14) {
                                tempSenString = tempSenString.replace("F", "")
                                tempSenString = Utils.ASCIItoHexString(tempSenString)
                            }
                            val newSenString = Utils.paddingWith(iThisField.toString(), "0", 3, false) + de57EncMethod + Utils.paddingWith(tempSenString.length.toString(), "0", 3, false) + tempSenString
                            de57HexString += newSenString
                        }
            ...
            if(de57HexString.length > 6){
                ...
                Utils.memcpy(clearByteDe57, tempDE57, tempDE57.size)
                ...
                certLog { "strlen(strClearDe57) :: ${Utils.strlen(strClearDe57)}" }
                ...
                try {
                    dukpt = dukptDeriv(Utils.hexStringToByteArray(strWekKey), newKsn)
                    dukpt.copyInto(TransData.dukpt)
                ...
                Utils.memcpy(encDe57, iEncDe57Len, encodedBytes, 0, encodedBytes.size)
                ...
                val eMode = "01"
                val tidLength = Utils.zeroPadding(TransData.tid.length.toString(), 2)
                val tleTid = Utils.paddingWith(TransData.tid, "0", 16, true)

                val hexTleHeader = Utils.ASCIItoHexString(applicationId + keyId + headerVer + encAlgo + uniqueKeyType + macAlgo + newKsn + delimiterChar + eMode + tidLength + tleTid + Dukpt.toHex(TleKcvDe57))
                val tleHeader = HexUtil.hexStringToByte(hexTleHeader)
                //TLE Header

                Utils.memcpy(de57, 0, tleHeader, 0, tleHeader.size)
                iDe57Len += tleHeader.size
                helperLog.appendLine(logClassName, "tleHeaderLen :: ${tleHeader.size}")

                Utils.memcpy(de57, tleHeader.size, encDe57, 0, iEncDe57Len)
        ...
        var strHash = Utils.hashDataWithClearText(HexUtil.bytesToHexString(macIsoByte, 7, macIsoByteLen - 7))
        helperLog.appendLine(logClassName, "hash :: $strHash")
        val tempHash = HexUtil.hexStringToByte(strHash)
        Utils.memcpy(hash, 0, tempHash, 0, tempHash.size)
        ...
        if (iHashPadlen != 0) {
            ...
            Utils.memset(hash, hashLen, 0.toByte(), iHashPadlen)
            hashLen += iHashPadlen
        ...
        try {
            ...
            Utils.memcpy(macByte, 0, bFullMac, bFullMac.size - 8, 8)
    ...
    fun parseIsoResp(helperLog: HelperLog, isoRespByte: ByteArray, isoRespByteOffset: Int, isoRespByteLen: Int) {
        ...
        var bf57 = TransData.getFromTransactionDb("BF57", 256)
        ...
        if(bf57.isNotEmpty()){
            ...
            helperLog.appendLine(logClassName, "Dukpt Hex :: ${Dukpt.toHex(TransData.dukpt)}")
            helperLog.appendLine(logClassName, "Decoded Base64 :: ${HexUtil.bytesToHexString(decodedBytes)}")

            try {
                strClearDe57resp = Encryption.decrypt(HexUtil.bytesToHexString(decodedBytes), Dukpt.toHex(TransData.dukpt), "DESede", "CBC")
                ...
                while (i > 3) {
                    ...
                    TransData.addHexStrIntoTransDB("BF" + String.format("%02d", tag), value)
    ...
    fun parseIsoRespIsolate(helperLog: HelperLog, isoRespByte: ByteArray, isoRespByteOffset: Int, isoRespByteLen: Int, returnByte: ByteArray, returnByteLenArr: IntArray, mapVariable: MutableMap<String, String>) {
        ...
        if(ptrLen > 0){
            //bf57 = Utils.byteArrayToHexString(ptrValue, 0 , ptrLen)
            //bf57 = Utils.byteArrayToAsciiString(Utils.hexStringToByteArray(bf57))
            bf57 = Utils.byteArrayToAsciiString(ptrValue, 0 , ptrLen)
        }
        ...
        if(bf57.isNotEmpty()){
            ...
            try {
                ...
                //TODO DUKPT
                helperLog.appendLine(logClassName, "Dukpt Hex :: $dukpt")
                helperLog.appendLine(logClassName, "Decoded Base64 :: ${HexUtil.bytesToHexString(decodedBytes)}")
 ...
 stan = IsoBatchInfoRepo.allocateCounter(context, "stan", schemeTag)
 ...
 invoiceNo = IsoBatchInfoRepo.allocateCounter(context, "invoiceNo", "pos")
        ...
        productModel?.let {
            ...
            try {
                ...
                ProductListRepo.updateData(context, hashMapOf("Ksn" to ksn), hashMapOf("AcqMid" to it.AcqMid, "AcqTid" to it.AcqTid))
            } catch (e: Exception){
                e.printStackTrace()
                ksn = ""
            }
        ...
        if(encDesField[0] != 0){
            ...
            if(de57HexString.length > 6){
                ...
                var dukpt = ByteArray(16)
                try {
                    dukpt = dukptDeriv(Utils.hexStringToByteArray(strWekKey), ksn)
                ...
                val delimiterChar = "01"
                val eMode = "01"
                val tidLength = Utils.zeroPadding(acqTid.length.toString(), 2)
                val tleTid = Utils.paddingWith(acqTid, "0", 16, true)

                val hexTleHeader = Utils.ASCIItoHexString(applicationId + keyId + headerVer + encAlgo + uniqueKeyType + macAlgo + ksn + delimiterChar + eMode + tidLength + tleTid + Dukpt.toHex(TleKcvDe57))
                val tleHeader = HexUtil.hexStringToByte(hexTleHeader)
                //TLE Header
