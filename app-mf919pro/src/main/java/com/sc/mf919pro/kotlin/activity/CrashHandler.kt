 ...
 * Utils.getIPAddress() -- and that threw on Android 10+, where getConnectionInfo() is nullable.
...
class CrashHandler : Thread.UncaughtExceptionHandler {
    ...
    private fun handOffRecovery(crashId: String, attempt: Int) {
        ...
        val intent = Intent(appContext, CrashRecoveryActivity::class.java).apply {
            ...
            putExtra(CrashRecoveryActivity.EXTRA_DELAY_MS, CrashState.backoffMsFor(attempt))
            putExtra(CrashRecoveryActivity.EXTRA_MAIN_PID, Process.myPid())
        }
    ...
    private fun requestReboot(): Boolean = try {
        val bundle = Bundle()
    ...
    private fun describeTransaction(): String = try {
        ...
        "type=${TransData.txnTypeLabel} stan=${TransData.stan} invoice=${TransData.invoiceNo} " +
            "batch=${TransData.batchNo} tid=${TransData.tid} ecr=$ecr"
    } catch (t: Throwable) {
        "unavailable"
    }
    ...
    companion object {
        ...
        private const val APP_PACKAGE = "com.sc.mf919pro"
        ...
        @Volatile
        private var cachedIp: String = Utils.UNKNOWN_IP
