...
public final class Dukpt {
    ...
    //ALIFTEST
    protected static byte[] ipekComputeKey(byte[] initPinEncrKey, byte[] keySerialNumber, BitSet keyRegisterBitmask, BitSet dataVariantBitmask) throws Exception {
        BitSet ipek = toBitSet(initPinEncrKey);
        BitSet ksn = toBitSet(keySerialNumber);
    ...
    public static String getNewKsn(String current_ksn){
        String newKsn = current_ksn;
        if(current_ksn != null){
            String sRunner = current_ksn.substring(current_ksn.length() - 5);
            int iRunner = Integer.parseInt(sRunner, 16);
            ++iRunner;
            if (iRunner >= 1048576) {
                iRunner = 0;
            }
            sRunner = Utility.zeroPadding(Integer.toString(iRunner, 16).toUpperCase(), 5);
            newKsn = current_ksn.substring(0, current_ksn.length() - 5) + sRunner;

            // secure memory
            byte[] ksnByte = toByteArray(newKsn);
            obliviate(ksnByte);
        }
    ...
    //--------[[CARDZONE ENHANCEMENT]]---------------------------------------------------
    public static byte[] xorByteArrays(byte[] array1, byte[] array2) {
        if (array1.length != array2.length) {
            throw new IllegalArgumentException("Byte arrays must be of the same length to XOR.");
        }
    ...
    public static byte[] decryptIpek(byte[] tmkBytes, byte[] encryptedIpekBytes) throws Exception {
        // Create the secret key from the TMK bytes
        SecretKey key = new SecretKeySpec(tmkBytes, "DESede");
        ...
        // Decrypt the IPEK
        return cipher.doFinal(encryptedIpekBytes);
    }

    public static String cardzoneRetailMac(String key, String data) {
        int len = key.length() / 2;
        String key1 = key.substring(0, len);
        String key2 = key.substring(len);

        if (data.length() % 16 == 0){
            System.out.println("SKIP Padding ISO!");
        } else {
            data = Utility.paddingWith(data, "0", data.length() + (16 - data.length() % 16), true);
            System.out.println("Padded ISO: " + data);
        }

        try {
            int loop = data.length() / 16;
            String output = data.substring(0, 16);
            data = data.substring(16);

            for(int j = 0; j < loop; ++j) {
                output = Cryptography.encrypt(output, key1, Cryptography.CryptoAlgorithm.DES);
                if (data.length() > 15) {
                    output = Cryptography.XORHexString(output, data.substring(0, 16));
                    data = data.substring(16);
                } else {
                    data = output;
                }
            }
            data = Cryptography.decrypt(data, key2, Cryptography.CryptoAlgorithm.DES);
            data = Cryptography.encrypt(data, key1, Cryptography.CryptoAlgorithm.DES);
    ...
    public static String encryptSenstiveData(String data, String encKey) {
        String result = "";
        try {
            // Step 1: Convert to hex
            //String hexString = Utility.ASCIItoHexString(data);
            String hexString = data;

            // Step 2: Chop into 8-byte blocks (16 hex characters), pad with zeros
            int blockLength = 16; // 8 bytes = 16 hex characters
            int totalBlocks = (hexString.length() + blockLength - 1) / blockLength;

            // Step 3: Encrypt each block with the same key
            StringBuilder encryptedString = new StringBuilder();
            for (int i = 0; i < totalBlocks; i++) {
                int start = i * blockLength;
                int end = Math.min(start + blockLength, hexString.length());
                String block = hexString.substring(start, end);
                String encryptedBlock = Encryption.encrypt(block, encKey, "DESede", "CBC");
                encryptedString.append(encryptedBlock);
            }
