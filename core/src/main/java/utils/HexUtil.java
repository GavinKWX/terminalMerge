...
public class HexUtil {
    ...
    public static String bcd2str(byte[] bcds, int offset, int bcdslen) {
        if (null == bcds) {
            return null;
        }
        ...
        try{
            char[] ascii = "0123456789abcdef".toCharArray();
            byte[] temp = new byte[bcdslen * 2];
            for (int i = 0; i < bcdslen; i++) {
                temp[i * 2] = (byte) ((bcds[offset+i] >> 4) & 0x0f);
                temp[i * 2 + 1] = (byte) (bcds[offset+i] & 0x0f);
            }
            StringBuffer res = new StringBuffer();
    ...
    public static byte[] Array(int iSource, int iArrayLen) {
    ...
    public static final String bytesToHexString(byte[] bArray, int offset, int length) {
        if (bArray == null){
            return null;
        }
