...
public final class QRCodeUtil {
    ...
    public static Bitmap createQRCode(String str, int widthAndHeight, Integer qrColor) throws WriterException {
        int qrBGColor = BLACK;
        if (qrColor != null) {
            qrBGColor = qrColor;
        }
    ...
    public static Bitmap createQrCodeBitmap(String content, int targetWidth, int targetHeight, Integer qrColor, Integer bgColor) throws WriterException {
        // 1. 创建目标尺寸的空白Bitmap
        Bitmap targetBitmap = Bitmap.createBitmap(targetWidth, targetHeight, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(targetBitmap);
        //canvas.drawColor(Color.WHITE); // 设置背景色
        canvas.drawColor(bgColor); // 设置背景色

        // 2. 直接生成与目标区域匹配的二维码（关键修改）
        Bitmap qrBitmap = createQRCode(content, targetHeight - 20, qrColor);

        if (qrBitmap == null) return targetBitmap;

        // 3. 计算居中坐标
        int left = (targetWidth - qrBitmap.getWidth()) / 2;
        int top = (targetHeight - qrBitmap.getHeight()) / 2;
