...
object IsoStepsGobiz {
    ...
    fun formIsoMessage(context: Context, helperLog: HelperLog, isoInfoModel: IsoInfoModel, destByteOffset: Int, isoByte: ByteArray, byteArrayLen: IntArray) {
        ...
        ByteOps.memcpy(de57, encVersion, 2)
        var iDe57Len = 2
        ...
        if(encDesField[0] != 0){
                    ...
                    when(iThisField){
                        35 -> {
                            ...
                            if (byteLastTrack2Byte.toInt() and 0x0F == 0x0F) {
                                ...
                                val newTrack2 = "0" + strTrack2.replace("F", "")
                                ...
                                ByteOps.memcpy(clearByteDe57, iClearByteDe57Len, bTrack2, 0, bTrack2.size)
                                iClearByteDe57Len += bTrack2.size
                            } else {
                                ByteOps.memcpy(clearByteDe57, iClearByteDe57Len, ptrValue, 0, iThisLen)
            ...
            if (iClearByteDe57Len > 8) {
                ...
                ByteOps.memcpy(clearByteDe57, bBmp, 8)
                ...
                try {
                    strEncDe57 = Encryption.encrypt(ByteOps.ASCIItoHexString(strClearDe57), strWekKey, "DESede", "CBC")
                } catch (e: Exception) {
                    e.printStackTrace()
                }
                ...
                ByteOps.memcpy(encDe57, iEncDe57Len, bEncDe57, 0, bEncDe57.size)
                ...
                ByteOps.memcpy(de57, 2, encDe57, 0, iEncDe57Len)
        ...
        println("ISO:BMP :: ${HexUtil.bytesToHexString(bBmp, 0, 8)}")
        ...
        try {
            ...
            repeat(desFieldLen){
                ...
                val iThisFieldLen = setField(iFieldId, CurrentTxn.transactionDb, tempValue, tempLen, false)
                if(iThisFieldLen > 0) {
                    ...
                    if(indexOfEnc == -1){
                        ...
                        ByteOps.memcpy(buffArray, 0, tempValue, 0, affectedLen[0])

                        ByteOps.memcpy(isoByte, isoByteLen, buffArray, 0, iThisFieldLen)
                        isoByteLen += iThisFieldLen
                        ByteOps.memcpy(macIsoByte, macIsoByteLen, buffArray, 0, iThisFieldLen)
                        macIsoByteLen += iThisFieldLen
                    }
                }
        ...
        certLog { "Masked MAC :: ${HexUtil.bytesToHexString(mac, 0, 8)}" }

        //ByteOps.memcpy(macIsoByte, macIsoByteLen, mac, 0, 8)
        //macIsoByteLen += 8
    ...
    fun formIsoSignOn(context: Context, helperLog: HelperLog, isoInfoModel: IsoInfoModel, prevIsoString: String, isoByte: ByteArray, byteArrayLen: IntArray): MutableMap<String, String>  {
        ...
        val strTmkKey = CurrentStore.secureValue(context, "eTmkId", refTag) ?: ""
        val hexTmkKey = HexUtil.hexStringToByte(strTmkKey)
        val strTakKey = CurrentStore.secureValue(context, "eTakKey", refTag) ?: ""
        // Decline rather than build DE57 and a MAC from empty keys.
        ...
        ByteOps.memcpy(de57, encVersion, 2)
        iDe57Len += 2
        ByteOps.memcpy(de57, iDe57Len, hexTmkKey, 0, hexTmkKey.size)
    ...
    fun parseRespSignOn(context: Context, helperLog: HelperLog, isoByte: ByteArray, byteArrayLen: IntArray): Boolean {
        ...
        ByteOps.memcpy(hexEncKey, 0, ptrValue, 10, ptrLen - 10)
        ...
        val strClearTmk = CurrentStore.secureValue(context, "eTmkKey", strSecureTleScheme) ?: ""
        ...
        ByteOps.memcpy(tpk, hexEncKey, 16)
        ByteOps.memcpy(wak, 0, hexEncKey, 16, 16)
        ByteOps.memcpy(wek, 0, hexEncKey, 16 + 16, 16)
        var strClearTpk = ""
        var strClearWak = ""
        var strClearWek = ""
        var clearTpk: ByteArray
        var clearWak: ByteArray
        var clearWek: ByteArray
        try {
            strClearTpk = Encryption.decrypt(HexUtil.bytesToHexString(tpk), strClearTmk, "DESede", "ECB")
            clearTpk = ByteOps.hexStringToByteArray(strClearTpk)
            strClearWak = Encryption.decrypt(HexUtil.bytesToHexString(wak), strClearTmk, "DESede", "ECB")
            clearWak = ByteOps.hexStringToByteArray(strClearWak)
            strClearWek = Encryption.decrypt(HexUtil.bytesToHexString(wek), strClearTmk, "DESede", "ECB")
            clearWek = ByteOps.hexStringToByteArray(strClearWek)
        } catch (e: Exception) {
            e.printStackTrace()
            return result
        }

        CurrentStore.storeSecureValue(context, "eTpkKey", strSecureTleScheme, clearTpk, clearTpk.size)
        CurrentStore.storeSecureValue(context, "eWakKey", strSecureTleScheme, clearWak, clearWak.size)
        CurrentStore.storeSecureValue(context, "eWekKey", strSecureTleScheme, clearWek, clearWek.size)
        result = true
        return result
    }
