...
public class EmvUtil {
    ...
    public static Bundle getTransBundle(String amount, String cashOutAmount, int channel, boolean isEnableContact, boolean isEnableContactless) {
        ...
        if(!cashOutAmount.equalsIgnoreCase("0.00")) {
            long salesAmount = utils.ByteOps.convertLong(amount.replaceAll("\\.", ""));
            long newCashOut = utils.ByteOps.convertLong(cashOutAmount.replaceAll("\\.", ""));
            long newAmount = salesAmount - newCashOut;
            amount = utils.AmountFormat.getActualAmount(Long.toString(newAmount));
    ...
    public static String readPan() {
        ...
        // D7 — tag 5A is a second source of the PAN and does not pass through readTrack2(), so a
        // card presenting 5A would otherwise leave the sink unarmed. Track 2 is left as whatever
        // readTrack2() registered; passing null here does not clear it.
    ...
    public static String getPbocData(String tagName, boolean isHex) {
        try {
            ...
            timber.log.Timber.tag(TAG).d("getPbocData Tag:" + tagName);
            int len = CurrentEmvHost.INSTANCE.emvHandler().readEmvData(new String[]{tagName.toUpperCase()}, data, new Bundle());
            if (len > 0) {
    ...
    public static String readTrack2() {
        ...
        timber.log.Timber.tag(TAG).d("readTrack2: " + LogRedact.track2(track2));
        return track2;
    }


    public static String getPanFromTrack2() {
    ...
    public static String getTLVDatas(String[] tags) {
        ...
        try {
            ...
            int byteNum = CurrentEmvHost.INSTANCE.emvHandler().readEmvData(tags, buffer, new Bundle());
            if (byteNum > 0) {
...
//        CardTagsEnum.getAcquirerChipTags(CurrentTxn.acqCode, CurrentTxn.schemeType)?.let {
//            for (lTag in it) {
...
//    }
    ...
    public static String getAcquirerRequiredTlvData() {
        String[] tags = CardTagsEnum.Companion.getAcquirerChipTags(iso.CurrentTxn.INSTANCE.getAcqCode(), iso.CurrentTxn.INSTANCE.getSchemeType());
        try {
            byte[] buffer = new byte[3096];
            int byteNum = CurrentEmvHost.INSTANCE.emvHandler().readEmvData(tags, buffer, new Bundle());
            if (byteNum > 0) {
