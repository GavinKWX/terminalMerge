...
import tms.models.QrPaymentAckRequestModel
import tms.models.QrPaymentAckResponseModel
import java.io.IOException

class QrPaymentAckHandler(private val envManager: EnvironmentManager) {
	private val className: String = (QrPaymentAckHandler::class.qualifiedName).toString()
	...
	fun invoke(
		...
		qrTxnRefNo: String,
		qrRefId: String
	): QrPaymentAckResponseModel {
		val gson = Gson()
		val qrPaymentAckRequestModel = QrPaymentAckRequestModel(
			TerminalInfo.sqnNum(),
		...
		val requestJsonString = gson.toJson(qrPaymentAckRequestModel)
		...
		val resp = HelperHttp.invokeSend(
			log,
			apiUrl = envManager.get(EnvironmentVariables::baseUrl) + "TMS/QR/PaymentAck",
			postData = requestJsonString,
			httpHeaders = httpHeaders
		)

		try {
			val gsonResp = gson.fromJson(resp, QrPaymentAckResponseModel::class.java)
