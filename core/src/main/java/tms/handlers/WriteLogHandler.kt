...
import tms.models.WriteLogRequestModel
import tms.models.WriteLogResponseModel
import java.io.IOException

class WriteLogHandler(private val envManager: EnvironmentManager) {
	private val className: String = (WriteLogHandler::class.qualifiedName).toString()
	...
	fun invoke(
		...
		deviceSN: String,
		action: String,
	): WriteLogResponseModel {
		val gson = Gson()
		val writeLogRequestModel = WriteLogRequestModel(deviceSN, action)
		...
		val resp = HelperHttp.invokeSend(
			log,
			apiUrl = envManager.get(EnvironmentVariables::baseUrl) + "tms/WriteLog",
			postData = requestJsonString,
			httpHeaders = httpHeaders
		)

		try {
			val gsonResp = gson.fromJson(resp, WriteLogResponseModel::class.java)
