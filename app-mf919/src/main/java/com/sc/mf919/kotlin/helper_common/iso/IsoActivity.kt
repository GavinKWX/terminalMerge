...
import com.sc.mf919.java.activity.IsoFiles
...
import com.sc.mf919.kotlin.database.repo.ProductListRepo.Companion.getUnSettledProduct
...
import com.sc.mf919.kotlin.helper_common.TmsHelper.sendWriteLog
import com.sc.mf919.kotlin.helper_common.iso.bsn.IsoStepsBSNNew
import com.sc.mf919.kotlin.helper_common.iso.bsn_cardzone.IsoStepsBsnCardZone
import com.sc.mf919.kotlin.helper_common.iso.gobiz.IsoStepsGobiz
import com.sc.mf919.kotlin.helper_common.iso.paydee.IsoStepsNew
...
object IsoActivity: Serializable {
    ...
    // HostTimeoutMs (default 60s) and the response is then parsed straight into the global
    // TransData. Anything that would reset or re-own TransData in that window corrupts a live
    // authorization, so callers gate on isHostRequestInFlight before doing so.
    // A counter, not a flag: settlement/batch-upload flows issue several sendToHost() calls
    // and a bool would be cleared by the first one to finish.
    ...
    /**
     * Mask cardholder data before it reaches the log file.
     *
     * magTrack2 is full magstripe track2 and the "Track2" line is actually DF02 (the PAN), so both
     * put a real card number into TerminaLog.txt, which uploadAllTerminalLog ships to TMS. Keeps
     * the masked PAN so a transaction is still identifiable; drops expiry, service code and
     * discretionary data. Note track3 is EMV tag data (8202...), not card data, and is left alone.
     */
    private fun maskPanLike(value: String): String {
        val sep = value.indexOfFirst { it == 'D' || it == 'd' || it == '=' }
        val pan = if (sep > 0) value.substring(0, sep) else value
        return Utils.hideCardDetails(pan) + if (sep > 0) "<redacted> (len=${value.length})" else ""
    }



    // Post-transaction persistence: batch, receipt, reversal and pre-auth rows. These all run AFTER
    // the customer may already have been charged, and DbHandler now propagates disk-full failures
    // (see isDiskFullOrIoError) instead of silently self-healing. Without a handler that throw is
    // uncaught on a bare IO coroutine and kills the process with the sale unrecorded -- the
    // "evaporated transaction" shape from the Aisino incident.
    //
    // A handler cannot save the row. What it does is convert a crash-after-charge into a logged,
    // reconcilable event, so the terminal stays up and the operator/back office has a record of what
    // to reconcile. The 2026-08-03 fix added a handler to updateSettlementValue only; these were bare.
    // Uses HelperLog's static API so it does not depend on a `log` being in scope at any call site.
    // See obsidian FIX-2026-08-04 (D3) and FIX-2026-08-05 (MF919).
    private fun persistenceScope(what: String): CoroutineScope =
        CoroutineScope(Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            try {
                val sb = HelperLog.init("IsoActivity - persistence")
                HelperLog.appendLine(sb, "PERSISTENCE FAILED ($what) :: $e")
                HelperLog.appendLine(sb, "Transaction may be missing from the terminal -- reconcile against host")
                HelperLog.logToFile(sb, HelperLogFileName.TerminaLogException)
            } catch (_: Exception) { /* never let the handler itself throw */ }
        })
    ...
    private fun invokeFunction(acqCode: String, functionName: String, vararg params: Any): Any? {
        ...
        if(classMethod != null){
            ...
            // 122-135 lines each and have no logToFile of their own, so their step flow used to
            // surface only inside the caller's block -- and died with it if the process was
            // killed mid-step. They all append to the caller's HelperLog, which is always one of
            // the params, so bracketing here covers every acquirer and every step in one place.
    ...
    fun processOnlineSale(context: Context, log: HelperLog) {
        // Refuse before the host is contacted AND before the STAN/invoice counters advance -- the
        // only point where nothing has happened yet and there is nothing to reconcile. Aisino wired
        // this into its plain online-sale path only; MF919 guards every financial entry point.
        // processSettlement is deliberately NOT guarded: blocking settlement on low space would trap
        // the batch on the device, which is the opposite of what we want when storage is short.
        if (!StorageGuard.canTransact(context)) {
            StorageGuard.logBlocked(context, "processOnlineSale")
            log.appendLine(logClassName, "ABORT processOnlineSale :: insufficient storage " +
                "(${StorageGuard.describe(context)}) -- transaction not started")
            log.logToFile(EnumLogFileName.TerminaLogException)
            TransData.transResult = Global.iso.err.txnNotAllowed
            // Surfaces as "Storage Full - Contact Support" on the result/acknowledge screen instead of a
            // bare "Failed". Also keeps the reversal loop in EmvActivity from firing on an empty code.
            TransData.respCode = Utils.ASCIItoHexString("ZS")
            return
        }
        ...
        acquirerIsoModel?.let { isoInfoModel ->
            ...
            val track2 = if (TransData.magTrack2Len > 0){
                ...
                log.appendLine(logClassName, "magTrack2 :: ${maskPanLike(magTrack2str)}")
                magTrack2str
            } else {
                EmvUtil.readTrack2()
            }
            ...
            if(thisIsoDbBufLen > 0) {
                ...
                if (TransData.transResult == Global.iso.err.txnApproved) {
                    persistenceScope("insertIntoBatchTable").launch {
                    ...
                    persistenceScope("insertIntoRevBatchTable").launch {
                        insertIntoRevBatchTable(context)
                    }
                }
            }
            persistenceScope("insertIntoPrintReceipt").launch {
                insertIntoPrintReceipt(context)
            }
    ...
    fun processPreauth(context: Context, log: HelperLog) {
        ...
        if (!StorageGuard.canTransact(context)) {
            ...
            log.appendLine(logClassName, "ABORT processPreauth :: insufficient storage " +
                "(${StorageGuard.describe(context)}) -- transaction not started")
            log.logToFile(EnumLogFileName.TerminaLogException)
            TransData.transResult = Global.iso.err.txnNotAllowed
            // Surfaces as "Storage Full - Contact Support" on the result/acknowledge screen instead of a
        ...
        acquirerIsoModel?.let { isoInfoModel ->
            ...
            if(thisIsoDbBufLen > 0) {
                ...
                log.appendLine(logClassName, "Transaction Result :: ${TransData.transResult}")
                if (TransData.transResult == Global.iso.err.txnApproved) {
                    persistenceScope("insertIntoPreauthTable").launch {
    ...
    fun processSaleComp(context: Context, log: HelperLog){
        ...
        acquirerIsoModel?.let {isoInfoModel ->
            ...
            if(thisIsoDbBufLen > 0) {
                ...
                persistenceScope("updateReceiptInfo").launch {
                    TransData.transEndDate = HelperCommon.getDateString(EnumDateFormat.yyyyMMddHHmmssSSS.dateFormat)
    ...
    //TODO Sale Completion with card presented (re-scan card through EMV).
    // Copied from processSaleComp as a starting point - adjust the card-presented specifics here.
    @JvmStatic
    @RequiresApi(Build.VERSION_CODES.O)
    fun processSaleCompCardPresented(context: Context, log: HelperLog){
        log.appendLine(logClassName, "-----------------Process Sales-Complete (Card Presented) [START]-------------------->")
        val acquirerIsoModel = IsoHelperNew.getIsoHelperObject(TransData.acqCode, "salecompcard")
        ...
        acquirerIsoModel?.let {isoInfoModel ->
            ...
            val tempInvoiceNo = IsoBatchInfoRepo.allocateCounter(context, "invoiceNo", "pos")
            TransData.invoiceNo = TransData.reqInvoiceNo
            log.appendLine(logClassName, "Invoice No :: ${TransData.invoiceNo}")
            ...
            TransData.addHexStrIntoTransDB("DF25", isoInfoModel.posCondition)
            TransData.addHexStrWithPadIntoTransDB("DF35",track2, "F")
            TransData.addHexStrIntoTransDB("DF37", HexUtil.str2HexStr(TransData.reqRrn))
            TransData.addHexStrIntoTransDB("DF38", HexUtil.str2HexStr(TransData.reqApprovalCode))
            TransData.addHexStrIntoTransDB("DF41", HexUtil.str2HexStr(TransData.tid))
            TransData.addHexStrIntoTransDB("DF42", HexUtil.str2HexStr(TransData.mid))
            TransData.addHexStrIntoTransDB("DF60", HexUtil.str2HexStr(TransData.batchNo))
            TransData.addHexStrIntoTransDB("BF60", HexUtil.str2HexStr(TransData.reqBatchNo))
            TransData.addHexStrIntoTransDB("DF62", HexUtil.str2HexStr(TransData.invoiceNo))
            val track3 = Utils.byteArrayToAsciiString(TransData.track3, 0 ,TransData.track3Len)
            log.appendLine(logClassName, "track3 :: $track3")
            TransData.addHexStrIntoTransDB("DF55", track3)
        ...
        log.appendLine(logClassName, "endTime :: $finish")
        log.appendLine(logClassName, "timeElapsed :: $timeElapsed")
        log.appendLine(logClassName, "-----------------Process Sales-Complete (Card Presented) [END]-------------------->")
        log.logToFile(EnumLogFileName.TerminaLog)
    }
    ...
    fun processVoidSale(context: Context, statusArray: BooleanArray, log: HelperLog){
        ...
        acquirerIsoModel?.let {isoInfoModel ->
            ...
            log.appendLine(logClassName, "Transaction Result :: ${TransData.transResult}")
            if (TransData.transResult == Global.iso.err.txnApproved) {
                persistenceScope("deleteVoidedInBatchTable").launch {
    ...
    fun processVoidPreauth(context: Context, statusArray: BooleanArray, log: HelperLog){
        ...
        acquirerIsoModel?.let {isoInfoModel ->
            ...
            log.appendLine(logClassName, "Transaction Result :: ${TransData.transResult}")
            if (TransData.transResult == Global.iso.err.txnApproved) {
                persistenceScope("deleteVoidedInPreauthTable").launch {
                    deleteVoidedInPreauthTable(context)
                    deleteVoidPreAuthInPrintReceipt(context)
                    insertIntoPrintReceipt(context)
                }
    ...
    fun processEppSale(context: Context, log: HelperLog) {
        ...
        if (!StorageGuard.canTransact(context)) {
            ...
            log.appendLine(logClassName, "ABORT processEppSale :: insufficient storage " +
                "(${StorageGuard.describe(context)}) -- transaction not started")
            log.logToFile(EnumLogFileName.TerminaLogException)
            TransData.transResult = Global.iso.err.txnNotAllowed
            // Surfaces as "Storage Full - Contact Support" on the result/acknowledge screen instead of a
    ...
    fun processAutoSettlement(log: HelperLog, mContext: Context, settleProduct: DbModelProductList, batchNo: String): Boolean {
        ...
        //TODO refundTxnTotal
        val summaryRecordRefundTxnTotal = SettlementSummaryRepo.getSelectiveData(
            mContext,
            ArrayList(listOf("acq_code", "mid", "tid", "tag")),
            arrayOf(settleProduct.AcqCode, settleProduct.AcqMid, settleProduct.AcqTid, "refundTxnTotal")
        )
        if (summaryRecordRefundTxnTotal.isNotEmpty()) {
            ...
            log.appendLine(logClassName, "\trefundTxnTotal = $refundTxnTotal")
        }
        //TODO refundTxnCount
        val summaryRecordRefundTxnCount = SettlementSummaryRepo.getSelectiveData(
            mContext,
            ArrayList(listOf("acq_code", "mid", "tid", "tag")),
            arrayOf(settleProduct.AcqCode, settleProduct.AcqMid, settleProduct.AcqTid, "refundTxnCount")
        )
        if (summaryRecordRefundTxnCount.isNotEmpty()) {
            ...
            log.appendLine(logClassName, "\trefundTxnCount = $refundTxnCount")
        }

        val settlementValueString = SettlementActivity.constructSettlementValueString(txnCount, txnTotal, refundTxnCount, refundTxnTotal, "")
    ...
    fun processSettlement(context: Context, settleProduct: DbModelProductList, settlementValueString: String, log: HelperLog): Boolean {
        ...
        persistenceScope("persistence").launch {
        ...
        findIsoModel?.let { isoModel ->
            ...
            for (transItem in batchTransList) {
                if(!ServiceHolder.autoSettlementIsRunning) {
                    TransData.transResult = Global.iso.err.txnNotCompleted
    ...
    fun processReversal(context: Context, prevResult: Boolean, isoInfoModel: IsoInfoModel, prevIsoString: String, isDeleteData: Boolean, log: HelperLog): Int? {
        ...
        if(!TransData.transIsoInfoModel?.reversalDes.isNullOrEmpty()) {
            log.appendLine(logClassName, "additional Reversal Des::${TransData.transIsoInfoModel?.reversalDes}")
            isoInfoModel.transactionDes = IsoHelperNew.mergeTransactionDes(isoInfoModel.transactionDes, TransData.transIsoInfoModel?.reversalDes)
        }
        ...
        if(tempLen > 0) {
            ...
            log.appendLine(logClassName, "stringRespCode :: $stringRespCode")
            responseCode = stringRespCode.toInt(16)
            persistenceScope("deleteSuccessReversal").launch {
                if(isDeleteData) {
    ...
    fun processMoto(context: Context, log: HelperLog) {
        ...
        if (!StorageGuard.canTransact(context)) {
            ...
            log.appendLine(logClassName, "ABORT processMoto :: insufficient storage " +
                "(${StorageGuard.describe(context)}) -- transaction not started")
            log.logToFile(EnumLogFileName.TerminaLogException)
            TransData.transResult = Global.iso.err.txnNotAllowed
            // Surfaces as "Storage Full - Contact Support" on the result/acknowledge screen instead of a
    ...
    fun sendToHost(context: Context, isoByteArray: ByteArray, isoByteArrayLen: Int, isReturnValue: Boolean, returnByte: ByteArray, returnByteArrayLen: IntArray, log: HelperLog){
        ...
        //val iConResp: Int
        /*if (acqSetting.isSSL) {
            isoRespLen = isoComm!!.sendToHostWithSSL(log, acqSetting.sslCert, primaryHostIP, primaryHostPort, primaryHostSSL, secondaryHostIP, secondaryHostPort, secondaryHostSSL, IsoFiles.getHostConnectTimeout(), timeoutMs, isoByteArray, 0, isoByteArrayLen, isoRespBuf, 0, 1024)
            iConResp = isoRespLen
        } else {
            isoRespLen = isoComm!!.sendToHost(log, primaryHostIP, primaryHostPort, secondaryHostIP, secondaryHostPort, IsoFiles.getHostConnectTimeout(), timeoutMs, isoByteArray, 0, isoByteArrayLen, isoRespBuf, 0, 1024)
            iConResp = isoRespLen
        }*/
        // Held from the moment the request goes out until the response has been parsed into
        // TransData: parseIsoResp writes the global, so releasing before it returns would
        // reopen the clobber window this counter exists to close. finally, not a trailing
        // decrement -- sendToHostWithSSL can throw, and a leaked count would wedge the
        // terminal into permanently refusing new transactions.
        hostRequestsInFlight.incrementAndGet()
        try {
            val isoRespLen = isoComm!!.sendToHostWithSSL(log, acqSetting.sslCert, primaryHostIP, primaryHostPort, primaryHostSSL, secondaryHostIP, secondaryHostPort, secondaryHostSSL, IsoFiles.getHostConnectTimeout(), timeoutMs, isoByteArray, 0, isoByteArrayLen, isoRespBuf, 0, 1024)
    ...
    suspend fun insertPendingTransactionInto(context: Context, txnType: String) = withContext(Dispatchers.IO){
        ...
        when (txnType) {
            ...
            "Debit Adjustment" -> {
                receiptTxnType = "DebitAdjustment"
            }
            "Credit Adjustment" -> {
                receiptTxnType = "CreditAdjustment"
            }
        }
    ...
    fun checkTransactionStatus(context: Context, log: HelperLog) {
        ...
        when(intRespCode){
            0x3030 -> {
                ...
                // Post-approval bookkeeping runs after the customer is already charged --
                // any uncaught exception here must never crash the terminal.
                val postApprovalErrors = CoroutineExceptionHandler { _, e ->
                    log.appendLine(logClassName, "POST-APPROVAL BOOKKEEPING FAILED :: $e")
                    log.logToFile(EnumLogFileName.TerminaLogException)
                }
        ...
        log.appendLine(logClassName, "-----------------Check Transaction Status [END]-------------------->")
        log.logToFile(EnumLogFileName.TerminaLog)
    }
    ...
    fun checkSettlementStatus(context: Context, settlementProduct: DbModelProductList, settlementValueString: String, log: HelperLog) {
        ...
        when(intRespCode){
            0x3030 -> {
                val store = DataStoreManager(context)
                TransData.transResult = Global.iso.err.txnApproved
                persistenceScope("initNewBatchNo").launch {
    ...
    suspend fun updateSettlementValue(context: Context, log: HelperLog, isReversal: Boolean) = withContext(Dispatchers.IO){
        ...
        var strTempRefundTxnTotal = ""
        var strTempRefundTxnCount = ""
        var iTempRefundTxnTotal: Long = 0
        var iTempRefundTxnCount = 0
        ...
        if(cardType == CardSchemeEnum.UNKNOWN) {
            ...
            log.appendLine(logClassName, "Track2 :: ${maskPanLike(track2)}")
        ...
        if(cardType != CardSchemeEnum.UNKNOWN) {
            ...
            // Safety net: this runs AFTER host approval, so it must never crash the app.
            // Should be unreachable once TransData ownership can't be clobbered mid-transaction
            // (see obsidian FIX-2026-08-03-TransData-Session-Clobber-Settlement-NPE) -- if this
            // fires, that guard failed and needs investigating, but the terminal must stay up.
            if(acqCode.isEmpty() || thisTxnMid.isEmpty() || thisTxnTid.isEmpty()){
                log.appendLine(logClassName, "SKIP updateSettlementValue :: incomplete TransData " +
                    "(acq='$acqCode' mid='$thisTxnMid' tid='$thisTxnTid' subtag='$settlementRefTag' " +
                    "inv=${TransData.invoiceNo} stan=${TransData.stan} rrn=${TransData.rrn} " +
                    "appr=${TransData.approvalCode} amt=${TransData.amount}) " +
                    "-- SETTLEMENT NOT UPDATED, reconcile required")
                log.logToFile(EnumLogFileName.TerminaLogException)
                return@withContext
            }

            fun readTag(tag: String): String? = SettlementSummaryRepo.getRecordValue(
                context, fieldList, arrayOf(acqCode, thisTxnMid, thisTxnTid, tag, settlementRefTag)
            )

            // txnTotal is the canonical row: absent means this acq/mid/tid/subtag combination
            // does not exist at all -- nothing to update, skip whole rather than half-apply.
            val canonicalTxnTotal = readTag("txnTotal")
            if(canonicalTxnTotal == null){
                log.appendLine(logClassName, "SKIP updateSettlementValue :: no SettlementSummary row for " +
                    "acq='$acqCode' mid='$thisTxnMid' tid='$thisTxnTid' subtag='$settlementRefTag' " +
                    "(inv=${TransData.invoiceNo} stan=${TransData.stan} rrn=${TransData.rrn} " +
                    "appr=${TransData.approvalCode} amt=${TransData.amount}) " +
                    "-- SETTLEMENT NOT UPDATED, reconcile required")
            ...
            // The combination exists, but an individual tag row can still be missing on
            // terminals upgraded before that tag was seeded -- default to 0 and note it.
            fun readOrZero(tag: String): String = readTag(tag) ?: run {
                log.appendLine(logClassName, "SettlementSummary tag row missing, defaulting to 0 :: " +
                    "'$tag'/'$settlementRefTag' (acq='$acqCode' mid='$thisTxnMid' tid='$thisTxnTid')")
                "0"
            }

            strTempTxnTotal = canonicalTxnTotal
            iTempTxnTotal = Utils.convertLong(strTempTxnTotal)
            strTempTxnCount = readOrZero("txnCount")
            iTempTxnCount = Utils.atoi(strTempTxnCount)

            strTempVoidTxnTotal = readOrZero("voidTxnTotal")
            iTempVoidTxnTotal = Utils.convertLong(strTempVoidTxnTotal)
            strTempVoidTxnCount = readOrZero("voidTxnCount")
            iTempVoidTxnCount = Utils.atoi(strTempVoidTxnCount)

            strTempCashOutTotal = readOrZero("cashOutTotal")
            iTempCashOutTotal = Utils.convertLong(strTempCashOutTotal)

            strTempRefundTxnTotal = readOrZero("refundTxnTotal")
            iTempRefundTxnTotal = Utils.convertLong(strTempRefundTxnTotal)
            strTempRefundTxnCount = readOrZero("refundTxnCount")
            iTempRefundTxnCount = Utils.atoi(strTempRefundTxnCount)
            ...
            log.appendLine(logClassName, "${TransData.schemeType} :: RefundTotal= $iTempRefundTxnTotal")
            log.appendLine(logClassName, "${TransData.schemeType} :: RefundCount= $iTempRefundTxnCount")
            ...
            } else if (TransData.txnTypeLabel.equals("Credit Adjustment", true)) {
                iTempRefundTxnTotal += txnAmount
                if (txnAmount > 0) {
                    iTempRefundTxnCount += 1
                } else {
                    log.appendLine(logClassName, "\t\t Note: ZERO AMOUNT not added to TxnCount")
                }
            } else {
            ...
            criteriaHM["tag"] = "refundTxnTotal"
            valueHM["value"] = iTempRefundTxnTotal.toString()
            SettlementSummaryRepo.updateData(context, valueHM, criteriaHM)
            criteriaHM["tag"] = "refundTxnCount"
            valueHM["value"] = iTempRefundTxnCount.toString()
            SettlementSummaryRepo.updateData(context, valueHM, criteriaHM)
            ...
            log.appendLine(logClassName, "New ${TransData.schemeType} :: RefundTotal= $iTempRefundTxnTotal")
            log.appendLine(logClassName, "New ${TransData.schemeType} :: RefundCount= $iTempRefundTxnCount")
        }
    }
    ...
    fun processCashOutSale(context: Context, log: HelperLog) {
        ...
        if (!StorageGuard.canTransact(context)) {
            ...
            log.appendLine(logClassName, "ABORT processCashOutSale :: insufficient storage " +
                "(${StorageGuard.describe(context)}) -- transaction not started")
            log.logToFile(EnumLogFileName.TerminaLogException)
            TransData.transResult = Global.iso.err.txnNotAllowed
            // Surfaces as "Storage Full - Contact Support" on the result/acknowledge screen instead of a
        ...
        log.appendLine(logClassName, "endTime :: $finish")
        log.appendLine(logClassName, "timeElapsed :: $timeElapsed")
        log.appendLine(logClassName, "-----------------Process CashOut-Sales [END]-------------------->")
        log.logToFile(EnumLogFileName.TerminaLog)
    }
    ...
    fun processCashOutVoid(context: Context, statusArray: BooleanArray, log: HelperLog){
        ...
        log.appendLine(logClassName, "endTime :: $finish")
        log.appendLine(logClassName, "timeElapsed :: $timeElapsed")
        log.appendLine(logClassName, "-----------------Process Void CashOut [END]-------------------->")
        log.logToFile(EnumLogFileName.TerminaLog)
    }

    @JvmStatic
    @RequiresApi(Build.VERSION_CODES.O)
    fun processAdjustment(context: Context, log: HelperLog, transactionType: String) {
        log.appendLine(logClassName, "-----------------Process Adjustment [START] ($transactionType) -------------------->")
        val acquirerIsoModel = IsoHelperNew.getIsoHelperObject(TransData.acqCode, transactionType)

        println("acqCode :: ${TransData.acqCode} :: $transactionType")
        println("acquirerIsoModel :: ${Gson().toJson(acquirerIsoModel)}")
        //TODO Check Speed
        log.appendLine(logClassName, "SchemeID :: ${TransData.schemeId}")
        acquirerIsoModel?.let { isoInfoModel ->
            TransData.transIsoInfoModel = isoInfoModel
            ...
            TransData.removeTlvFromTransDb("DF25")
            TransData.addHexStrIntoTransDB("DF25", isoInfoModel.posCondition)
            //TODO 60
            //TransData.addHexStrIntoTransDB("DF60", HexUtil.str2HexStr(TransData.batchNo))
            //TransData.addHexStrIntoTransDB("DF62", HexUtil.str2HexStr(TransData.invoiceNo))
            ...
            log.appendLine(logClassName, "strPosEntryMode :: $strPosEntryMode")
            TransData.addHexStrIntoTransDB("DF22", strPosEntryMode)*/
            //TODO sync with other change to db method
        ...
        log.appendLine(logClassName, "endTime :: $finish")
        log.appendLine(logClassName, "timeElapsed :: $timeElapsed")
        log.appendLine(logClassName, "-----------------Process Adjustment [END] ($transactionType) -------------------->")
        log.logToFile(EnumLogFileName.TerminaLog)
    }
