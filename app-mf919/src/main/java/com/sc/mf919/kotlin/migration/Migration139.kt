...
class Migration139 {
	...
	* - Add VOID_WITH_PIN column into TerminalConfiguration
	* */
	/*
	...
	* */
	...
	fun startMigrate139(){
		sbLog = HelperLog.init("----- Start Running Migration 139 -----")
		...
		/*  Table Exist */
		HelperLog.appendLine(sbLog, "Check Column in (TerminalConfiguration) -> VOID_WITH_PIN")
		if (!dbHandler.checkTableColumnExist(DatabaseTables.TERMINAL_CONFIGURATION, "VOID_WITH_PIN")) {
			HelperLog.appendLine(sbLog, "Column Not Exist")
			alterNewTerminalConfigurationTables(dbHandler);
		}

		HelperLog.appendLine(sbLog, "Finish Running Migration 139")
	...
	private fun renameExistingTableToBackup(dbHandler: DbHandler, tableName: String) {
		...
		val query = "ALTER TABLE " + tableName + " RENAME TO " + tableName + "_BKV139"
		dbHandler.runningCustomSql(query, arrayOf())
	}
	...
	private fun alterNewTerminalConfigurationTables(dbHandler: DbHandler) {
		...
		val tableCol = "Contact, Contactless, MagStripe, ForcePin, IsoPrint, ReceiptPrint, Sale, Void, PreAuth, SaleCom, SaleComOnline, Refund, OptIn, TmsReceipt, TmsEnable, AutoSettle, QrPay, PoweredBy, PoweredByBW, HomeLogo, MC_VER, DEV_PROJECT, DEV_LOCATION, DEV_LANE_ID";
		...
		) //  TerminalConfiguration > TerminalConfiguration_BKV139
