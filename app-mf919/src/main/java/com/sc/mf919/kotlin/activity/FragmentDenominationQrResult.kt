...
class FragmentDenominationQrResult: Fragment() {
...
//        linearLayout.background.colorFilter = BlendModeColorFilterCompat.createBlendModeColorFilterCompat(Color.GRAY, BlendModeCompat.SRC_ATOP)
        view.findViewById<LinearLayout>(R.id.buttonGroupLeft).setOnClickListener {
            if (this::helperLog.isInitialized) {
                helperLog.appendLine(helperLogClassName, "Selected :: Done [DENOMINATION QR RESULT]")
                helperLog.logToFile(EnumLogFileName.TerminaLog)
            }
            listener?.fragmentDenominationBackAction()
        }

        val transStatus = if (TransData.qrRespCode == "0000") {
            openNfc()
            "Approved"
        } else {
...
//            tvProduct.setTextSize(TypedValue.COMPLEX_UNIT_SP, 15f)
        ...
        renderListItem("Channel", "Duitnow")
        renderListItem("Date/Time", Utils.DateTimeFormat(transDateAsci))
        renderListItem("QR Ref ID", TransData.qrRef)
        renderListItem("Ref No", TransData.qrTxnRef)
        renderListItem("Response Code", TransData.qrRespCode)
        renderListItem("Description", transStatus)
        startTimer(0)
    }
...
//        AisinoHelper.openNfcUrlInterface("https://", "${environmentManager.get(EnvironmentVariables::nfcUrl)}$aesData")
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        if (this::helperLog.isInitialized) {
            helperLog.appendLine(helperLogClassName, "FragmentDenominationQrResult OnDestroyView :: denomination QR result screen ended")
            helperLog.logToFile(EnumLogFileName.TerminaLog)
        }
    }
}
