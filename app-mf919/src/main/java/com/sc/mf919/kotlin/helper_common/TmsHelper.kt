...
import com.sc.mf919.kotlin.database.repo.*
...
object TmsHelper {
	...
	private fun checkSettlementSummary(log: HelperLog, mContext: Context, modelSummary: DbModelSettlementSummary) {
		...
		if (dbData == null) {
			val tagList = listOf("txnTotal", "txnCount", "voidTxnTotal", "voidTxnCount", "cashOutTotal", "refundTxnTotal", "refundTxnCount")
		...
		val dupeCount = SettlementSummaryRepo.checkDuplicate(mContext)
		log.appendLine(className, "Checking SettlementSummary Duplicate :: [$dupeCount]")
		if (dupeCount > 1) {
			val result = SettlementSummaryRepo.deleteDuplicate(mContext)
			log.appendLine(className, "Clearing Duplicates :: [$result]")
		}
	}
	...
	fun getMerchantConfiguration(log: HelperLog, mContext: Context): Boolean {
		...
		var respBatchNo = 0
		var respInvoiceNo = 0
		log.appendLine(className, "TMS MerchantConfig [START]")
		...
		try {
			...
			productArray.forEach { arrayItem ->
				if (arrayItem == "CARD_SETTINGS") {
					if (cardSetting != null) {
						...
						checkSettlementSummary(log, mContext, dbModelSettlementSummary)
					}
					return@forEach
			...
			result = true
			log.appendLine(className, "TMS MerchantConfig [END] :: RESP_CODE=${apiResp.RESP_CODE} " +
					"RESP_DESC=${apiResp.RESP_DESC} MC_VER=${apiResp.MC_VER}")
			log.logToFile(EnumLogFileName.TerminaLog)
		} catch (ex: Exception) {
			...
			try {
				...
				errorResponse?.let {
					if(it.RESP_CODE == "9000"){
						...
						log.appendLine(className, "Merchant not configured (9000) - product listing cleared, sales blocked")
					}
				}
			} catch (jsonEx: Exception) {
				log.appendLine(className, "Json Exception in Error -> ", jsonEx.toString())
			}
			log.appendLine(className, "TMS MerchantConfig [END] :: failed")
			log.logToFile(EnumLogFileName.TerminaLogException)
		}
	...
	fun getTerminalConfiguration(log: HelperLog, mContext: Context): Boolean {
		var result = false
		log.appendLine(className, "TMS TerminalConfig [START]")
		val environmentManager = EnvironmentManager(Helper.getInstance().getPrefs()!!)
		...
		try {
			...
			result = true
			log.appendLine(className, "TMS TerminalConfig [END] :: RESP_CODE=${terminalConfigRes.RESP_CODE} " +
					"RESP_DESC=${terminalConfigRes.RESP_DESC} MC_VER=${terminalConfigRes.MC_VER}")
			log.logToFile(EnumLogFileName.TerminaLog)
		}catch (ex: Exception){
			log.appendLine(className, "TerminalConfigHandler Response (Exception)", ex.toString())
			log.appendLine(className, "TMS TerminalConfig [END] :: failed")
			log.logToFile(EnumLogFileName.TerminaLogException)
		}
		return result
	}
