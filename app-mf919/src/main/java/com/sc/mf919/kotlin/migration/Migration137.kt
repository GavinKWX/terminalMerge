...
class Migration137 {
	...
	* */
	...
	fun startMigrate137(){
		sbLog = HelperLog.init("----- Start Running Migration 137 -----")
		...
		/*  Table Exist */
		HelperLog.appendLine(sbLog, "Check Column in (printReceiptQr) -> txnApprCode")
		if (!dbHandler.checkTableColumnExist(DatabaseTables.PRINT_RECEIPT_QR, "txnApprCode")) {
			HelperLog.appendLine(sbLog, "Column Not Exist")
			alterNewPrintReceiptQrTables(dbHandler);
		}

		HelperLog.appendLine(sbLog, "Check Column in (qrPayTable) -> txnApprCode")
		if (!dbHandler.checkTableColumnExist(DatabaseTables.QR_PAY_TABLE, "txnApprCode")) {
			HelperLog.appendLine(sbLog, "Column Not Exist")
			alterNewQrPayTables(dbHandler);
		}
		HelperLog.appendLine(sbLog, "Finish Running Migration 137")
	...
	private fun renameExistingTableToBackup(dbHandler: DbHandler, tableName: String) {
		...
		val query = "ALTER TABLE " + tableName + " RENAME TO " + tableName + "_BKV137"
		dbHandler.runningCustomSql(query, arrayOf())
	}
	...
	private fun alterNewPrintReceiptQrTables(dbHandler: DbHandler) {
		...
		val tableCol = "id, postingDt, txnType, payBrand, txnDt, mid, tid, txnAmt, refId, hostRefNo, txnRefNo, respCode, status, addInfo, printInfo, acqCode";
		...
		) //  PrintReceiptQr > PrintReceiptQr_BKV137
	...
	private fun alterNewQrPayTables(dbHandler: DbHandler) {
		...
		val tableCol = "id, postingDt, txnType, payBrand, txnDt, seqNo, mid, tid, txnAmt, refId, hostRefNo, txnRefNo, status, addInfo, acqCode";
