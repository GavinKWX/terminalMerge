...
public class IsoComm {
    ...
    private static void sysPrint(String message, byte[] data, int dataOffset, int dataLen) {
        Utils.debugLogPrint(TAG, message + HexUtil.bytesToHexString(data, dataOffset, dataLen));
    }
    ...
    private int sslTcpConnect(int certId, String ip, int port, int timeoutms) {
        ...
        try {
            InputStream sslInputStream = ServiceHolder.Companion.getContext().getResources().openRawResource(certId);
            ...
            sysPrint("Exception in loading ssl certificate");
            e.printStackTrace();
        }

        if (sslContext == null) {
            sysPrint("SSL context is null, aborting SSL connection");
            return -1;
        }
        ...
        try {
            *//*SSLSocketFactory sf = sslContext.getSocketFactory();
            ...
            // Do handshake NOW so first write isn't delayed by TLS negotiation
            ssl.startHandshake();
            ...
            sysPrint("Connection Fail...(" + ip + ":" + port + ")...");
            e.printStackTrace();
        }*/
        ...
        try {
            ...
            try (InputStream sslInputStream = sslFile.exists()
                    ...
                    : ServiceHolder.Companion.getContext().getResources().openRawResource(certId)) {
    ...
    private void tcpDisconnect() {
        try {
            /*if (!isConnected) {
                Utils.debugLogPrint(TAG, "socket not created");
                return;
            }*/
    ...
    private int tryConnectPrimarySecondary(boolean useSSL, int certID, String ip1, String port1, Boolean port1SSL, String ip2, String port2, Boolean port2SSL, HelperLog helperLog) {
        ...
        if (iSid < 0) {
            ...
            if (iSid < 0) {
                ...
                helperLog.appendLine(logClassName, connectionStatus);
                return Global.iso.err.connectionFailed;
            } else {
                connectionStatus = "Connected with Secondary connection";
                helperLog.appendLine(logClassName, connectionStatus);
            }
        } else {
            // Success on the primary path is already covered by "Connected to IP", so
            // silence here means primary worked. The Secondary branch above still logs,
            // so a failover stays visible.
    ...
    public int sendToHost(HelperLog helperLog, String ip1, String port1, String ip2, String port2, int connectTimeoutms, int timeoutms, byte[] data, int dataoffset, int dataLen, byte[] resp, int respOffset, int respMaxsize) {
        ...
        if (iSid < 0) return Global.iso.err.connectionFailed;
        ...
        while (tickTimeMs > 0) {
            ...
            Utils.DelayMili(1000); // delay 1 second
        }
        ...
        try {
            ...
            helperLog.appendLine(logClassName, "Timeout...");
            RespLen = -1;
            return Global.iso.err.communicationTimeout;
        } catch (Exception e) {
