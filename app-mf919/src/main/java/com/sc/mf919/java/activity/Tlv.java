...
public class Tlv implements Serializable
{
    ...
    {
        Utils.debugLogPrint("TLV",message);
    }
    ...
    private static void sysPrint(String message, byte[] data, int dataOffset, int dataLen)
    {
        ...
        Utils.debugLogPrint("TLV", HexUtil.bcd2str(data, dataOffset, dataLen));
    }
    ...
    public int encodeTag(String tag, byte[] dest, int destOffset)
    {
        ...
        //Utils.debugLogPrint(TAG, "encodeTag: " + tag + "-->" + bTag.length);
    ...
    public int encodeLen(int len, byte[] dest, int destOffset)
    {
        ...
        else if(len<0x010000)
        {
            ...
            destOffset++;
            Utils.set_ushort(len, dest, destOffset);
            return 3;
        }
        else
            return -1;
    ...
    public int encodeValue(byte[] value, int valueoffset, int valuelen, byte[] dest, int destOffset)
    {
        ...
        //Utils.printLog("value=" + HexUtil.bytesToHexString(value, valueoffset, valuelen));
        if(valuelen>0)
        {
            Utils.memcpy(dest, destOffset, value, valueoffset, valuelen);
        }
    ...
    public int decodeLen(byte[] lenptr, int lenptrOffset, int[] affectedlen)
    {
        ...
        return Global.iso.err.invalidInputLen;//invalid len coding
    }
    ...
    public String decodeTag(byte[] tagptr, int tagptrOffset, int[] affectedlen)
    {
        ...
        while(tagptr[iPointer] == 0x00)//Paypass3.0 Test Case 3M50-0005(send_old_tornTrx_inDiscData) - 0xFF is not NULL
        {
            ...
            {
                Utils.printLog("TLV:TagNotFound: Exceeded MAX NULL(2)");
    ...
    public int getNextTag(byte[] datain, int datainOffset, int datainlen, String[] tag, int[] len_offset, int[] len_size, int[] value_offset, int[] value_size)
    {
        ...
        while(datain[iPointer] == 0x00)//Paypass3.0 Test Case 3M50-0005(send_old_tornTrx_inDiscData) - 0xFF is not NULL
        {
            ...
            {
                //hh_debug_printdec("tag null ended", iPointer);
                Utils.printLog("TLV:TagNotFound: Exceeded input limit");
                return Global.tlv.err.tagNotFound;//global.tlv.err.tagNotFound;
            }

            if(iPointer > maxNullLen)//max check len = 32 --> need more than 200 for ISO engine
            {
                Utils.printLog("TLV:TagNotFound: Exceeded MAX NULL");
                return Global.tlv.err.tagNotFound;//global.tlv.err.tagNotFound;//no tag found
            }
            if(datain.length<=iPointer)
            {
                return Global.tlv.err.invalidLen;
            }
        }
        if(iPointer>0)
        {
        ...
        // Get Tag
        //
        //Utils.debugLogPrint("GetTag:", HexUtil.bytesToHexString(datain));
        //Utils.debugLogPrint("GetTag1:", String.valueOf(iPointer));
        strTag = decodeTag(datain, iPointer, iAffectedLength);
        //Utils.debugLogPrint("GetTag2:", Arrays.toString(iAffectedLength));
        if(iAffectedLength[0]>(datainlen-iPointer))
            return Global.tlv.err.tagNotFound;//global.tlv.err.tagNotFound;// no more tags (finish)
        if(strTag == null)
            return Global.tlv.err.tagNotFound;//global.tlv.err.tagNotFound;//no tag found
        ...
        if(iLen < 0)
            return Global.tlv.err.invalidLen;//global.tlv.err.invalidLen;//Invalid Length
        if(iAffectedLength[0]>(datainlen-iPointer))
            return Global.tlv.err.invalidLen;//global.tlv.err.invalidLen;// no more tags
        ...
        if(iLen>(datainlen-iPointer))
            return Global.tlv.err.invalidValue;//global.tlv.err.invalidValue;// Invalid Data
    ...
    public int getTlv(byte[] datain, int datainlen, String tag, byte[] value, int[] endedoffset)
    {
        ...
        while(true)
        {
            ...
            if (strTag[0] == tag) {
                ...
                Utils.memcpy(value, 0, datain, iPosition + iValueOffset[0], iValueOffset[0] + iValueSize[0]);
    ...
    public int getValue(byte[] datain, int datainOffset, int datainlen, String tag, byte[] value, int[] endedOffset)
    {
        ...
        while(true)
        {
            ...
            //Utils.debugLogPrint("TAG", "getValue:  --->" + iPosition);
            iRet = getNextTag(datain, iPosition, datainlen, strTag, iLenOffset, iLenSize, iValueOffset, iValueSize);
            //Utils.debugLogPrint("TAG", "getValue: " + Arrays.toString(strTag));
            //Utils.debugLogPrint("TAG", "getValue: " + Arrays.toString(iLenOffset) +"---"+ Arrays.toString(iLenSize) +"---"+ Arrays.toString(iValueOffset) +"---"+ Arrays.toString(iValueSize));
            ...
            {
                return Global.tlv.err.invalidLen;//global.tlv.err.invalidLen;
            }
            ...
            if (strTag[0].equals(tag))
            {
                ...
                {
                    Utils.memcpy(value, 0, datain, /*iPosition + */iValueOffset[0]+1, /*iValueOffset + */iValueSize[0]);
                }
                else
                {
                    Utils.memcpy(value, 0, datain, /*iPosition + */iValueOffset[0], /*iValueOffset + */iValueSize[0]);
                }
                //hh_debug_printbytes("Yes. Return value1", datain + iValueOffset, iValueSize);
                //hh_debug_printbytes("Yes. Return value2", *value, iValueSize);
                //Utils.debugLogPrint("TLVTAG", HexUtil.bytesToHexString(value));
    ...
    public int markoffTag(byte[] datain, int datainOffset, int datainlen, String tag)
    {
        ...
        while(true)
        {
            ...
            if (strTag[0].equals(tag))
            {
                //Utils.debugLogPrint("TAG", "markoffTag: " + HexUtil.bytesToHexString(datain));
                byte[] tmp=new byte[(datainlen-(iRet - iPosition))];
                //byte[] tmp=new byte[(datainlen- iRet)];
                Utils.memset(datain, iPosition/*iValueOffset*/, (byte) 0, iRet - iPosition);
                ...
                //Utils.memset(datain, tmp.length-1/*iValueOffset*/, (byte) 0, iRet - iPosition);
                Utils.memset(datain, tmp.length/*iValueOffset*/, (byte) 0, iRet - iPosition);
                int count = Integer.parseInt(HexUtil.bytesToHexString(new byte[]{datain[0],datain[1]}),16);
                //Utils.debugLogPrint("TAG", "markoffTag: " + count + "--" + iRet + "--" + iPosition);
                byte[] hexCount=HexUtil.hexStringToByte(Utils.zeroPadding(Integer.toString((count-(iRet-iPosition)),16),4));
                datain[0]=hexCount[0];
                datain[1]=hexCount[1];
                //Utils.debugLogPrint("TAG", "markoffTag: " + HexUtil.bytesToHexString(datain));
